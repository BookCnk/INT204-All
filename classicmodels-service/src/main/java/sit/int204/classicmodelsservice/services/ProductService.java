...
//        return repository.findByProductLineStartingWith(category);
    ...
    public Page<Product> getProducts(String partOfProduct, Double lower, Double upper,
        ...
        // ตรวจสอบเงื่อนไขสำหรับราคา lower และ upper
        if (lower <= 0 && upper <= 0) {
            // หาค่าราคาสูงสุดและกำหนดให้เป็น upper
            upper = repository.findFirstByOrderByPriceDesc().getPrice();
        }
        if (lower > upper) {
            // สลับค่า lower และ upper ในกรณี lower มากกว่า upper
            double tmp = lower;
            lower = upper;
            upper = tmp;
        }
        ...
        if (sortBy != null && sortBy.length > 0) {
            ...
            // สร้างหน้าผลลัพธ์ด้วยการเรียงลำดับและการแบ่งหน้า
            ...
            // สร้างหน้าผลลัพธ์โดยใช้การแบ่งหน้าเท่านั้น
